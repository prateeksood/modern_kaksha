...
const registerTeacherService = async (data, callBack)=>{
    ...
    try{
        ...
        if (foundTeachers.length===0) { 
            ...
            jwt.sign({ _id: savedTeacher._id },'my_jwt_secret_123',{ expiresIn: tokenExpireTime},async (err, token) => {
...
const registerStudentService = async (data, callBack)=>{
    ...
    try{
        ...
        if (foundStudents.length===0) { 
            ...
            jwt.sign({ _id: savedStudent._id },'my_jwt_secret_123',{ expiresIn: tokenExpireTime},async(err, token) => {
                if (err) {return callBack(err, 400)}
