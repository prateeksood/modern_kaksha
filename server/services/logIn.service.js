...
const logInTeacherService = async (data,callBack)=>{
    ...
    if (foundTeacher.length>0) {
        ...
        .then(isMatched => {
            if (isMatched) {
                jwt.sign({ _id: foundTeacher[0]._id }, 'my_jwt_secret_123',{ expiresIn: 31536000 },(err, token) => {
...
const logInStudentService = async (data,callBack)=>{
    ...
    if (foundStudent.length>0) {
        ...
        .then(isMatched => {
            if (isMatched) {
                jwt.sign({ _id: foundStudent[0]._id }, 'my_jwt_secret_123',{ expiresIn: 31536000 },(err, token) => {
